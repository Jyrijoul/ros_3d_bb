...
class RViz:
    ...
    def text(self, uid=0, x=0.0, y=0.0, z=1.1, text="", scaling=0.3, duration=1):
        if text == "":
            text = str(uid)
        marker = Marker(
    ...
    def arrow(self, uid=0, x=0.0, y=0.0, v_x=0.0, v_y=0.0, duration=1):
        ...
        marker = Marker(
            ...
            header=Header(frame_id='map'),
            color=ColorRGBA(1.0, 0.0, 0.0, 0.9))
        self.marker_pub.publish(marker)
